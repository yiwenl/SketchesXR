...
import vs from 'shaders/typhoons.vert';
import fs from 'shaders/typhoons.frag';

import data from './typhoons.json';

var random = function(min, max) { return min + Math.random() * (max - min);	}

class ViewTyphoons extends alfrid.View {
	...
	_init() {
		const posOffset = [];
		const extras = [];
		// const dataCopy = data.concat().filter((o, i) => i % 2 === 0);
		const dataCopy = data;
		// dataCopy.length = 500;
		console.log('Number of data : ', dataCopy.length);
		...
		dataSets.forEach( dataSet => {
			...
			const mesh = alfrid.Geom.sphere(0.0002, 12);
