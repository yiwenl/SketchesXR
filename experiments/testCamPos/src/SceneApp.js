...
class SceneApp extends Scene {
  ...
  _onTouch = () => {
    ...
    if (mtxHit !== null) {
      ...
      vec3.transformMat4(this.pointOrigin, this.pointOrigin, this.mtxHit);
    }
  };
  ...
  sendCameraData(mCameraPos, cameraFov) {
    // Create an object with camera data
    const cameraPos = vec3.create();
    vec3.sub(cameraPos, mCameraPos, this.pointZero);

    // keep the save height level
    cameraPos[1] = mCameraPos[1];

    const cameraData = {
      cameraPos,
      cameraFov,
      pointZero: this.pointZero,
      cameraPosOrg: mCameraPos,
    };

    // Emit the data to the server
    this.socket.emit("cameraData", cameraData);
  }

  render() {
    if (this._hasPresented) {
      this.sendCameraData(this.camera.position, this.camera.fov);
    }

    if (Math.random() > 0.9 && this._hasPresented) {
      console.log(
        "Camera Position, fov : ",
