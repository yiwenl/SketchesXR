...
void main(void) {
    ...
    float _senseRadius = mix(1.0, 2.0, extra.z) * radiusScale;
    ...
    // noise
    vec3 noise = curlNoise(pos * 2.0 + uTime);
    acc += noise * 0.2;
    ...
    acc += dir * f * mix(0.5, 1.5, extra.y) * 2.0;

    float speedOffset = mix(0.95, 1.0, extra.x);
    vel += acc * 0.00025 * speedOffset *  uOffsetTouch;;
