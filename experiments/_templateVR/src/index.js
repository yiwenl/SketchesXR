...
import * as VRUtils from "./VRUtils";
...
function _init3D() {
  ...
  if (isDevelopment && !VRUtils.isVRSupported) {
    Settings.init();
  }
...
function checkAR() {
  VRUtils.checkSupported().then((supported) => {
...
function initStartButton() {
  ...
  btnAR.addEventListener("click", () => {
    ...
    VRUtils.init(GL.gl).then((gl) => {
      document.body.removeChild(canvas);
      // scene.present();

      VRUtils.onSessionEnd(() => {
